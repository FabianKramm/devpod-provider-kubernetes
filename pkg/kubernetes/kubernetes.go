...
func NewKubernetesDriver(options *options.Options, log log.Logger) (driver.Driver, error) {
...
func (k *KubernetesDriver) deletePod(ctx context.Context, podName string) error {
	err := k.client.Client().CoreV1().Pods(k.namespace).Delete(ctx, podName, metav1.DeleteOptions{
